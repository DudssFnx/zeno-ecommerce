...
async function main() {
  ...
  const orders = await db.query(
    `SELECT id, order_number, total FROM orders ORDER BY id`,
  );
  const report: any[] = [];
  ...
  for (const o of orders.rows) {
    ...
    if (res.rowCount > 1) {
      report.push({
        ...
        total: o.total,
        receivables: res.rows,
      });
    } else if (res.rowCount === 1) {
      const r = res.rows[0];
      // Check installments and sum
      const insts = await db.query(
        `SELECT id, amount, remaining FROM installments WHERE receivable_id = $1 ORDER BY due_date`,
        [r.id],
      );
      const sum = insts.rows.reduce(
      ...
      if (Number(o.total) !== sum) {
        report.push({
          ...
          total: o.total,
          receivableId: r.id,
          issue: "installments_mismatch",
          installments: insts.rows,
        });
      }
    }
  }
